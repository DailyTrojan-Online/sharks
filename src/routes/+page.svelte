<script lang="ts">
    ...
    const weights = [
        9.1, 6.7, 6.3, 6.1, 6, 4.3, 4, 2.8, 2.4, 2.4, 2.2, 2, 2, 1.9, 1.5, 1,
        0.8, 0.15, 0.15, 0.1, 0.07,
    ];
    ...
    let wordsLeftToFind = $state(0);
    $inspect(wordsLeftToFind);
    let sharkInterval = 4;
    ...
    let progressDotOffset: number = $state(0);

    function offsetProgressDots() {
        if (progressDotWrapper == null) return;
        let wrapRect = progressDotWrapper.getBoundingClientRect();
        progressDotOffset = wrapRect.width / 2 - 10;
        progressDotOffset -= 35 * wordsFound;
    }
</script>
...
<style>
    ...
    .dot {
        ...
        border-radius: 50px;
        flex-shrink: 0;
        transform: translateX(var(--tx)) scale(40%);
        display: flex;
        align-items: center;
        justify-content: center;
    ...
    .dot-shark {
        background: transparent;
        fill: red;
        transform: translateX(var(--tx)) scale(100%);
    }
    .dot-shark * {
        fill: red !important;
    }
    ...
    @keyframes finSwim {
        ...
        45% {
            left: calc(
                100vw - 110px - 60px +
                    ((100vw - min(500px, calc(100vw - 80px))) / -2)
            );
            transform: rotateY(0deg);
        }
        50% {
