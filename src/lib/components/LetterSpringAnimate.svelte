<script lang="ts">
    ...
    let letterScale = new Spring(.2, {
        stiffness: 0.1,
        damping: 0.25,
    });
    let letterAngle = new Spring(-80 * (Math.random() - .5), {
        stiffness: 0.1,
        damping: .15,
    });
...
<div class="letter-wrapper" style:width={`${letterWidthSpring.current}px`}>
	<div class="letter" bind:this={letterElement} style:transform={`translate(-50%, -50%) rotateZ(${letterAngle.current}deg) scale(${letterScale.current})`}>
		{letter}
	</div>
</div>
