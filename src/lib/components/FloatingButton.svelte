<script lang="ts">
    ...
    $effect(() => {
        if (disabled && buttonContainer) {
          let rect = buttonContainer.getBoundingClientRect();
          particles.spawnAtPoint(rect.left * window.devicePixelRatio, rect.top * window.devicePixelRatio, rect.width * window.devicePixelRatio, rect.height * window.devicePixelRatio, 100)
        }
    });

    let buttonContainer: HTMLElement | null = $state(null);
</script>
